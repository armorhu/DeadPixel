package starling.extensions.advancedjoystick
{
	...
	public class JoyStick extends Sprite
	{
		...
		[Embed(source="skins/default_stick.png")]
		private static const dsClassEmbed:Class;
		public static const DEFAULT_STICK:Bitmap=new dsClassEmbed();
		[Embed(source="skins/default_holder.png")]
		private static const dhClassEmbed:Class;
		public static const DEFAULT_HOLDER:Bitmap=new dhClassEmbed();
		...
		private static const errorMessage:String="You are trying to initialize a JoyPad with the " + "default skin, but you have removed the the lines of code " + "that Embed this skin. Please uncomment the Embed Code.";

		public function getHolderTexture():Texture
		{
			return stick.texture;
		}

		public function JoyStick(holderTexture:Texture=null, stickTexture:Texture=null, activateImmediately:Boolean=true, scale:Number=1)
		{
			super();
			if (holderTexture == null && JoyStick["DEFAULT_HOLDER"])
				holderTexture=Texture.fromBitmap(JoyStick["DEFAULT_HOLDER"], true, false, scale);
			if (stickTexture == null && JoyStick["DEFAULT_STICK"])
				stickTexture=Texture.fromBitmap(JoyStick["DEFAULT_STICK"], true, false, scale);
		...
		*/
		...
		{
			if (stick.hasEventListener(TouchEvent.TOUCH))
				return;
		...
		*/
		private function initialize(holderTexture:Texture, stickTexture:Texture, activateImmediately:Boolean=true):void
		{
			if (holder)
			{
				holder.dispose();
